...
- ETFirst：A 股 ETF 实时价格、IOPV、溢价率、成交额、更新时间和 NDX 日线。
- AkShare：HaoETF 历史缺失时，用 AkShare ETF 日线收盘价与历史单位净值生成近似历史溢价。
- HaoETF：QDII 历史溢价，仍作为历史溢价优先源。
- Yahoo Finance/yfinance：NQ 期货实时/历史盘中行情。

ETFirst 实时行情按接口实际单位换算价格和 IOPV，并严格校验交易日、更新时间及溢价率复算结果。数据源可能缺失、延迟或字段变化；凡是参与规则的关键字段不可用，系统返回 `SKIP_DATA`，不会用旧数据、猜测数据或 LLM 补全。
